...
public class PlateJadeTooltipProvider implements IBlockComponentProvider, IServerDataProvider<BlockAccessor> {
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor blockAccessor, IPluginConfig iPluginConfig) {
        if (blockAccessor.getBlockEntity() instanceof PlateBlockEntity) {
            NbtList itemList = blockAccessor.getServerData().getList(PLATE_ITEM_KEY, NbtElement.COMPOUND_TYPE);
            IElementHelper elementHelper = IElementHelper.get();
            ListIterator<NbtElement> it = itemList.listIterator();
            int rows = 0; //前面已有的行数
            while (it.hasNext()) {
                if (it.next() instanceof NbtCompound stackEntry) {
                    ItemStack stack = ItemStack.fromNbt(stackEntry);
                    if(stackEntry.contains(REAL_COUNT_KEY)){
                        stack.setCount(stackEntry.getInt(REAL_COUNT_KEY)); //byte会溢出成负数，还原真实值
                    }
                    if (stack.isEmpty()) continue;
                    if (rows < 4 || blockAccessor.getPlayer().isSneaking()) {
                        //未到达上限或按住Shift，全画
                        IElement icon = elementHelper.item(stack, 0.6f)
                                .size(new Vec2f(10, 10))
                                .translate(new Vec2f(-1.5f, -1.7f))
                                .message(null);//关闭叙述
                        tooltip.add(icon); //add在新的一行加
                        tooltip.append(elementHelper.spacer(2, 0));//高度由行高决定
                        int count = stack.getCount();
                        if (count > 1) {
                            tooltip.append(Text.literal(count + " x "));
                        }
                        tooltip.append(stack.getName()); //append紧跟着加
                        rows++;
                    } else {//已有4行且玩家未按下Shift键
                        tooltip.add(Text.translatable("tooltip.quincyplate.and_so_on", itemList.size() - 4));
                        break;
                    }
                }
            }
    ...
    @Override
    public void appendServerData(NbtCompound data, BlockAccessor blockAccessor) { //传递服务端数据的方法
        if (blockAccessor.getBlockEntity() instanceof PlateBlockEntity plate) {
            ...
            for (PlateContentItem contentItem : contents) {
                ...
                for (var stack : toolTipData) { //有一样的就合并
                    if (ItemStack.canCombine(stack, item)) {//不比较数量，只比较物品和数据
                        stack.increment(1);
                        merged = true;
                        break;
                    }
            ...
            NbtList transmittedData = new NbtList();
            for (ItemStack infoStack : toolTipData) { //把整理后的、仅含物品信息不含坐标信息的数据装进data里
                NbtCompound serializedNBT = infoStack.writeNbt(new NbtCompound());
